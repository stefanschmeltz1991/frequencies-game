...
class Frequency:
    ...
    def addPlayer(self, playerNumber):
        self.player_list.append( Player(self.game.COLOR_LIST[playerNumber],
                                 self.game.UNIT_COSTS[playerNumber],
                                 self.loadUnitImages(playerNumber)) )
        ...
        for x in range(max(1, square.x - 2), 1 + min(18, square.x + 2)):
            for y in range(max(1, square.y - 2), 1 + min(18, square.y + 2)):
                ...
                if s.getPlayer() == self.getActivePlayer():
                    if isinstance(s.unit, Soldier) and s.distance(square) == 1:
