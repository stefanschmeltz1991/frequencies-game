...
greybackground   = loadImage("greyimage.jpg")
...
MENU_POSITION = [ (0.94, 0), (24.5, 0), (0.94, 9.9), (24.5, 10) ]
...
class Board:
    ...
    def buildPlayerMenu(self, playerNumber):
        ...
        button_y = ( y + line_distance * 7.3 )

        #CREATION OF THE BUY ACTION BUTTON, THE FUNCTIONALITY, THE IMAGES, THE IMAGES WHEN THE MOUSE IS HOVERING OVER THE ACTIONBUTTON, THE POSITION OF THE ACTIONBUTTON
        self.hover_list[playerNumber].append(ActionButton(self.game.buyAction,
                                                          colors,
                                                          buy,
                                                          buyw,
                                                          ((x - 0.1) * offset, button_y * offset), buttonSize))
        #CREATION OF THE BUY ACTTACK BUTTON, THE FUNCTIONALITY, THE IMAGES, THE IMAGES WHEN THE MOUSE IS HOVERING OVER THE ACTIONBUTTON, THE POSITION OF THE ACTIONBUTTON
        self.hover_list[playerNumber].append(ActionButton(self.game.attackAction,
                                                          colors,
                                                          attack,
                                                          attackglow,
                                                          ((x + 1.5) * offset, button_y * offset), buttonSize))
        #CREATION OF THE BUY NEXTBUTTON, THE FUNCTIONALITY, THE IMAGES, THE IMAGES WHEN THE MOUSE IS HOVERING OVER THE ACTIONBUTTON, THE POSITION OF THE ACTIONBUTTON
        self.hover_list[playerNumber].append(ActionButton(self.game.nextAction,
                                                          colors,
                                                          nextturn,
                                                          nextturnw,
                                                          ((x + 3.1) * offset, button_y * offset), buttonSize))
  ...
  #boardsquare is een subclass
        #code reuse
        # elke keer dat een action button wordt gemaakt
        #alle button  zijn encapsulations
class Button:
...
class BoardSquareButton(Button):
    ...
    def removeUnit(self):
        #self.unit = None
        self.board.placeUnit(self.x, self.y, vierkant) # the unit will be removed bij replacing the unit with an empty image
    def removeUnit2(self):
        self.unit = None
        self.board.placeUnit(self.x, self.y, vierkant2)
    def distance(self, square):
        return max(abs(square.x - self.x), abs(square.y - self.y))# abs Return the absolute value of the argument.
...
def shiftPosition(pos, x, y):# pos is a tulpe, this functionality of this function is to shift the position of the image that hovers over the units
    xy = list(pos)
    return (int(xy[0] + (x/2)), int(xy[1] + (y/2)))
    
    
class UnitButton(Button):#subclass of the superclass Button.
